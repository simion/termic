...
//   { kind: "new" } → spawn a fresh agent tab with the workspace's default CLI,
 ...
 *  with an explicit CLI (falls back to the workspace default when omitted). */
...
function sendToAgent(wsId: string, prompt: Prompt, tabId: string) {
  const target = (useApp.getState().tabs[wsId] ?? []).find(
  ...
  if (busy) {
    useApp.getState().enqueueAgentMessage(wsId, target.id, prompt.body);
  ...
  useApp.getState().patchTab(wsId, target.id, { lastInputAt: Date.now() });
  useUI.getState().pushToast(`Sent "${prompt.title}" to ${label}.`, "success");
}

function spawnAgentWithPrompt(wsId: string, prompt: Prompt, explicitCli?: string) {
  const s = useApp.getState();
  const tabs = (s.tabs[wsId] ?? []) as Tab[];
  // Use the chosen CLI; otherwise the workspace default (default agent tab's
  ...
  const tick = () => {
    const t = (useApp.getState().tabs[wsId] ?? []).find(t => t.id === newTabId);
    if (t && t.type === "terminal" && t.ptyId) {
      window.setTimeout(() => {
        ...
        const still = (useApp.getState().tabs[wsId] ?? []).find(t => t.id === newTabId);
        if (!still || still.type !== "terminal" || !still.ptyId) return;
        sendMessageToPty(still.ptyId, prompt.body);
        useApp.getState().patchTab(wsId, newTabId, { lastInputAt: Date.now(), promptPendingTitle: null });
    ...
    if ((useApp.getState().tabs[wsId] ?? []).some(t => t.id === newTabId)) {
      useApp.getState().patchTab(wsId, newTabId, { promptPendingTitle: null });
...
export function runPrompt(wsId: string, prompt: Prompt, dest: PromptDest): void {
  if (dest.kind === "new") spawnAgentWithPrompt(wsId, prompt, dest.cli);
  else sendToAgent(wsId, prompt, dest.tabId);
}
