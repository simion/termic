...
export function PromptLibrarySection() {
  ...
  async function confirmReset(id: string, title: string) {
    const ok = await useUI.getState().askConfirm({
      title: t("prompts.resetTitle", { title }),
      message: t("prompts.resetMessage"),
  ...
  async function confirmRestore() {
    const ok = await useUI.getState().askConfirm({
      ...
      message: t("prompts.restoreMessage"),
      confirmLabel: t("prompts.restoreLabel"),
    });
    ...
    <div>
      <div className="flex items-start justify-between gap-4">
        ...
        <Button variant="primary" size="sm" className="shrink-0 gap-1.5" onClick={openNewPrompt}>
          <Plus className="h-3.5 w-3.5" /> {t("prompts.new")}
        </Button>
      </div>
        ...
        {prompts.map(p => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {/* Row actions */}
              <IconBtn title={t("prompts.editTip")} onClick={() => setEditingId(p.id)}>
                <Pencil className="h-4 w-4" />
              </IconBtn>
              <IconBtn title={p.enabled ? t("prompts.disableTip") : t("common:enable")} onClick={() => toggleEnabled(p.id)}>
                {p.enabled ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
              </IconBtn>
              <IconBtn title={t("prompts.duplicateTip")} onClick={() => clonePrompt(p.id)}>
                <Copy className="h-4 w-4" />
              </IconBtn>
              {p.builtin && p.modified && (
                <IconBtn title={t("prompts.resetTip")} onClick={() => confirmReset(p.id, p.title)}>
                  <RotateCcw className="h-4 w-4" />
                </IconBtn>
              )}
              <IconBtn title={t("common:delete")} danger onClick={() => deletePrompt(p.id)}>
            ...
            >
              ...
              >
                {previewText(p.body) || t("prompts.emptyPrompt")}
              </span>
            </button>
          </div>
        ))}
      ...
      {deletedCount > 0 && (
        <div className="mt-4">
          ...
          >
            {t("prompts.restore", { count: deletedCount })}
          </button>
        </div>
      )}
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11.5px] text-[var(--color-fg-faint)]">
              {incomplete ? t("prompts.incomplete") : t("prompts.autosave")}
            </span>
            <div className="flex items-center gap-2">
